...
- **マルチワークスペースサポート**：各ワークスペースフォルダに独立した MCP サーバーインスタンスを提供
- **グローバル設定ファイル**：共有グローバル一時ファイルを使用してワークスペース設定を管理し、複数の Cursor インスタンスをサポート
- **クラス定義検索**：Maven 依存関係から Java クラス定義を検索
- **依存関係一覧**：プロジェクトのすべての Maven 依存関係を一覧表示
- **ソースコード抽出**：利用可能な場合、`-sources.jar` ファイルからソースコードを抽出
...
```
VSCode 拡張機能 (MCP クライアント)
├── ワークスペースフォルダ 1 → MCP サーバーインスタンス 1 (stdio)
├── ワークスペースフォルダ 2 → MCP サーバーインスタンス 2 (stdio)
└── ワークスペースフォルダ N → MCP サーバーインスタンス N (stdio)
```

各ワークスペースフォルダには独立した MCP サーバープロセスが割り当てられ、以下を保証します：
- 独立した状態とキャッシュ
- ワークスペース固有の Maven 設定
- 異なるプロジェクト間の干渉なし
...
1. **ワークスペース検出**：拡張機能はワークスペースフォルダの変更を監視し、Maven プロジェクトを検出します
2. **MCP サーバー生成**：各 Maven プロジェクトに対して、独立した MCP サーバープロセスが生成されます
3. **依存関係解決**：MCP サーバーは `pom.xml` を解析し、Maven ローカルリポジトリ（`~/.m2/repository`）内の JAR ファイルを特定します
4. **クラス抽出**：クラスが要求された場合：
   - まず依存関係の `-sources.jar` を探します
   - 見つかった場合、Java ソースコードを直接抽出します
   - それ以外の場合、コンパイルされたクラスファイルの解析にフォールバックします
5. **キャッシュ**：結果は LRU キャッシュを使用してキャッシュされ、パフォーマンスが向上します

## 設定

### 自動設定（VSCode 拡張機能）

VSCode 拡張機能を使用する場合、自動的に：
- `pom.xml` を探して Maven プロジェクトを検出
- 各ワークスペースフォルダの MCP サーバーを起動
- Maven リポジトリパスで MCP サーバーを設定
...
**マルチワークスペースサポート：**
- ツールに異なる `pomPath` パラメータを渡すことで、複数のワークスペースがサポートされます
- 各ツール呼び出しで異なる `pomPath` を指定して、異なる Maven プロジェクトで作業できます
- 別々の MCP サーバーインスタンスやグローバル設定ファイルは不要です

### 設定の詳細

拡張機能は自動的に以下を検出します：
- Maven ローカルリポジトリパス（デフォルト：`~/.m2/repository`）

**マルチワークスペースの使用：**
- ツール呼び出しで `pomPath` パラメータを使用して、使用する Maven プロジェクトを指定します
- 例：プロジェクト 1 の場合、`pomPath: "/path/to/project1/pom.xml"` で `find_class_definition` を呼び出し
- 例：プロジェクト 2 の場合、`pomPath: "/path/to/project2/pom.xml"` で `find_class_definition` を呼び出し
...
拡張機能は MCP リソースも提供します：

- **`maven://dependencies?pomPath={pomPath}`**：プロジェクトのすべての Maven 依存関係の JSON リストを返します。例：`maven://dependencies?pomPath=/path/to/project/pom.xml`
...
皆様のサポートにより、このプロジェクトを継続的に改善し、開発者コミュニティ向けにより有用なツールを作成できます。中国人です、日本語を勉強しています。
